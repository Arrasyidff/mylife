...
export function SummaryStat({ label, value, delta, deltaTone, large, icon, className }: SummaryStatProps) {
  return (
    <div
      className={`min-w-0 rounded-xl ${
        large
          ...
          : 'p-[14px_16px] md:p-[16px_18px] bg-white border border-[#E0EAE6]'
      } ${className ?? ''}`}
    ...
    >
      ...
      <div className={`font-bold mt-1.5 tracking-[-0.5px] tabular-nums ${large ? 'text-[22px] md:text-[28px]' : 'text-[17px] md:text-[22px]'}`}>
        {value}
      </div>
      {delta && (
